    ...
    useEffect(() => {
        const fetchTrafficData = async () => { // if no props, then no traffic data
            ...
            try {
                ...
                // calculate the bounding box of the origin and destination, bbox parameter for incidents api
                ...
                // Formula: Area ≈ 111.32 * (lat2 - lat1) * 111.32 * (lon2 - lon1) * cos((lat1 + lat2) / 2)
                const latDistance = north - south;
                const lngDistance = east - west;
                const middleLat = (north + south) / 2;
                const latDifference = latDistance * 111; //convert to km
                const lngDifference = lngDistance * 111 * Math.cos(middleLat * Math.PI / 180);
                const area = latDifference * lngDifference;
                
                console.log("Bounding box area:", area, "km²");
                ...
                if (area > 10000) {
                    ...
                    const shrinkFactor = Math.sqrt(10000 / area);
                    const newLatRangeKm = latDiffKm * shrinkFactor;
                    const newLonRangeKm = lonDiffKm * shrinkFactor;
                    
                    // convert back to degrees for coordinates calculation
                    const newLatRangeDeg = newLatRangeKm / 111;
                    const newLonRangeDeg = newLonRangeKm / (111 * Math.cos(midLat * Math.PI / 180));
                ...
                const incidentsResponse = await fetch(
                    ...
                    `fields=incidents(type,geometry(type,coordinates),properties(iconCategory,location,delay,severity,status,startTime,endTime,length))&` +
                ...
                const congestionPercentage = Math.round((1 - currentSpeed / freeFlowSpeed) * 100);
