export async function POST(req) {
  ...
  try {
    ...
    const textBody = {
      textQuery: userResponses.textQuery
        ? `${userResponses.textQuery} near ${address}`
        : null, //this should always have a value unless it is searched by url
      openNow: true,
      regionCode: "US",
      languageCode: "en",
    ...
    userResponses.priceLevel
      ? (textBody.priceLevels = [userResponses.priceLevel])
      : null;
    ...
    const headers = {
      ...
      "X-Goog-FieldMask":
        "places.displayName,places.formattedAddress,places.rating,places.photos,places.priceRange",
    };
    ...
    if (data.places) {
      //this section is to get the photos to be able to display. Each photo is a get request
      await Promise.all(
        data.places.map(async (eachService) => {
