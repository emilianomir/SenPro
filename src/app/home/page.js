                        ...
                        <div className=" mt-10 mb-3 py-3">
                            ...
                            <div className="flex justify-center w-full overflow-x-auto whitespace-nowrap">
                                {cardInfo.map((information, index)=> (
                                    <div key = {index} className="w-full md:w-1/4 mr-3 inline-block">
                                        <div onClick={()=>getServices(index)} className="w-full h-85 bg-land-card mt-3 ml-3 rounded-lg group cursor-pointer">
                                            ...
                                            <div className="h-1/3 flex bg-gray-300/50 rounded-b-lg group-hover:bg-gray-400/50 ">                        
                                                <div className="h-full flex flex-col pl-2">
                                                    <div className="font-bold text-xl text-left text-content-text h-1/2 w-full text-wrap">
                                                        {information.header}
                                                    </div>
                                                    <div className="grid grid-cols-12 h-1/4">
                                                        ...
                                                        <div className="flex items-center col-end-9 md:col-end-11">
                                                            <div>
                ...
                <div className={`${openOverlay != null ? "opacity-100": "opacity-0"} transition-opacity ease-in-out duration-300 bg-land-sec-bg p-6 rounded-lg shadow-lg w-5/6 h-4/5 relative`}>
                    ...
                    <div className="w-full flex flex-col justify-center">
                        ...
                        <div className="mt-3 w-full flex items-center justify-center">
                            <div className="max-sm:w-25 max-sm:h-25 max-w-50">
                                <Image
                        ...
                        <div className="text-center text-base md:text-xl px-3 mt-3 mb-5 text-content-text">Scan the QR Code above on your phone for Google Maps Link. Or Click <a className="text-blue-400 hover:underline" href={cardInfo[openOverlay].dir} target="_blank" rel="noopener">Here </a></div>
                    </div>
                    </> }
                    <div className="overflow-y-scroll h-2/5 whitespace-nowrap">
                    { openOverlay != null &&
                
                    (cardInfo[openOverlay].done ? 
                    cardInfo[openOverlay].services.map((service, index) => (
                        <div className="ml-4 w-9/10 h-4/5 md:h-4/5 max-sm:pt-3 mb-4" key = {service.id}>
                            <div className="w-full bg-history-card/80 rounded-xl h-4/5 shadow-lg">
                                <div className="grid grid-cols-3 max-sm:grid-rows-2 md:grid-cols-12 w-full h-full gap-2 ">
                                    ...
                                    <div className={`${service.displayName.text.length > 25 ? "col-span-2 md:col-span-7" : "col-span-2 md:col-span-6"} flex flex-col h-full justify-center pl-5`}>
                                        <div className="font-extrabold text-xs text-wrap md:text-3xl text-content-text rounded-lg mb-2">
                                            {service.displayName.text}
                                        </div>
                                        <div className="text-content-text/70 font-semibold text-[12px] text-wrap md:text-2xl">
                                            {service.formattedAddress}
                                        </div>
                                    </div>
                                    <div className="col-span-1 md:col-span-2 md:col-end-12 flex justify-center items-center w-full">
                                        <p className = "bg-blue-600 text-white text-center text-[10px] md:text-2xl font-semibold px-2 py-1.5 rounded-sm ms-3 w-full"> 
                                            Rating: {service.rating ? service.rating : "N/A" }
                                                <svg className="w-2 md:w-4 h-2 md:h-4 text-yellow-300 inline mb-1 ml-1" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 20">
